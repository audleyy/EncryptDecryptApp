...
int caesar_encrypt(const uint8_t* in, size_t in_len, int shift, uint8_t* out, size_t out_cap, size_t* out_len) {
...
int caesar_decrypt(const uint8_t* in, size_t in_len, int shift, uint8_t* out, size_t out_cap, size_t* out_len) {
    int check = validateParams(in, in_len, out, out_cap, out_len);
    if (check != CAESAR_OK) return check;
